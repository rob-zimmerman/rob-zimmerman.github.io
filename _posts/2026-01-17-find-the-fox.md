...
It's worth noting that this method does introduce a tiny amout of sampling bias. This is because when we take a valid 1-$\FOX$ grid and work backwards, it must have been generated from a $\FOX$less grid in which the now-$\FOX$ was previously a different $3$-letter string. There are $25$ possible such strings, as we saw earlier. However, not all $25$ of those strings would necessarily yield a $\FOX$less grid; depending on the letters surrounding the string, inserting some of those $25$ strings could accidentally complete a <i>different</i>, overlapping $\FOX$. Therefore, 1-$\FOX$ grids whose surrounding letters restrict the number of $\FOX$less predecessors will be just slightly less likely to be generated by our procedure. The difference, however, is negligible.
